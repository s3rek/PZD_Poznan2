...
Ext.onReady(function() {
	...
	var pageLayer = new OpenLayers.Layer.Vector("RamkaDruk", {
	...
	//warstwy 
	//warstwa działek 
	var saveStrategyDzialki = new OpenLayers.Strategy.Save({
		auto: true
	});
	wfsDzialki = new OpenLayers.Layer.Vector("Działki Rodzinne", {
	...
	//warstwa ogrodow 
	var saveStrategyOgrody = new OpenLayers.Strategy.Save({
		auto: true
	});

	wfsOgrody = new OpenLayers.Layer.Vector("Ogrody Rodzinne", {
	...
	var saveStrategyZdjecia = new OpenLayers.Strategy.Save({
		auto: true
	});
	...
	var warstwyPanel = new Ext.tree.TreePanel({
		title: "Warstwy",
		plugins: [
	...
	function properAttrWin(feature) {
		...
		if (_lname == "Działki Rodzinne") {
			_lname = 'dz'
		} else if (_lname == "Ogrody Rodzinne") {
			_lname = 'o'
		} else if (_lname == "Warstwa Powierzchniowa") {
			_lname = 'wpow'
		} else if (_lname == "Warstwa Punktowa") {
			_lname = 'wpkt'
		} else if (_lname == "Warstwa Liniowa") {
			_lname = 'wlin'
		} else if (_lname == "Zdjęcia Działkowców") {
			_lname = 'wzd'
		}
		//if (_upr){
		if ((readCookie('poz2_upr')) && (readCookie('poz2_usr'))) {
	...
	function createPopup(feature, _selectControl, nazwaFunkcjiGenForm, _title) {
		...
		if (bLeft) {
			popupPos = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(mapExtent.left + (leftOffset * reso), mapExtent.top - (topOffset * reso)))
		} else {
			popupPos = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(mapExtent.right - (rightOffset * reso), mapExtent.top - (topOffset * reso)))
		}
