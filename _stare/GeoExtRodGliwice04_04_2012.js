...
OpenLayers.ProxyHost= "RODGliwice/proxy.php?url=";
...
//no-comment
var mapPanel, warstwyPanel, warstwyDodatkowe, map, wfsDzialki, wfsOgrody, measureControls, desc, logBttn, _usr, _upr, popup, aktywnyOgrod, nenene, mappanel11, printPage11;
...
*/
    ...
    // our page.
    printPage11 = new GeoExt.data.PrintPage({
        printProvider: printProvider
    });

    // The map we want to print
    mapPanel11 = new GeoExt.MapPanel({
		region: "center",
		//layers: [new OpenLayers.Layer.WMS("Tasmania", "192.168.20.15:8081/geoserver/RODGLIWICE/wms",
        //    {layers: "RODGLIWICE:dzialki"}, {singleTile: true})],
		map:map,		
    ...
    */
    ...
    pageLayer.addFeatures(printPage.feature);
		...
		function druk()	{
				...
				drukP.show();
				var _drukPanel = drukP.cloneConfig();
				rightPanel.add(drukP.cloneConfig());
				rightPanel.doLayout();
	...
	var drukP = new Ext.form.FormPanel({
        ...
        bbar: ["->", {
            ...
            handler: function() {
                ...
                printPage11.fit(mapPanel11, true);
                // print the page, optionally including the legend
                printProvider.print(mapPanel11, printPage11, includeLegend && {legend: legendPanel});
            }
        }, {
            xtype: "checkbox",
    ...
    map.events.register('moveend', this, function() {
		...
		var stylZdjecia = new OpenLayers.StyleMap({
	    	"default": new OpenLayers.Style({
							externalGraphic: "RODGliwice/ikony/fullsize/${heading}.png",
							graphicHeight: 20,
							graphicWidth: 20,
							graphicOpacity: 1
				}),
		...
		if(wararray!=null){
			...
			var adresWMS = 'RODGliwice/dajWMSLayers.php';
 ...
 //czyta ciasteczko o nazwie name
		    ...
		    if ((readCookie('upr')) && (readCookie('usr'))) { 
		    	logBttn.setText('Wyloguj');
				  ...
				  //znaczki w hex w caisteczku sie zapisaly
					...
					var adres = 'RODGliwice/dajBufor.php?iddeleg=' + _upr.getGid('d') + '&idogr=' + _upr.getGid('o') + '&iddz=' + _upr.getGid('u');
					Ext.Ajax.request({
							url: adres,
							success: function(objServerResponse, opts) {									
									var odp = Ext.decode(objServerResponse.responseText);
		            ...
		            login.getForm().submit({	 
		                ...
		                success: function(){
				                Ext.Msg.alert('Status', 'Poprawnie Zalogowano!', function(btn, text){		
									...
									animeInt = setInterval("liczCzas()", 1000);	 
				                    //if (btn == 'ok'){
				                    //    window.location = '';  //ta lokalizacja w ktorej teraz jestesmy 'zdjecia' przeniosloby do http://127.0.0.1/RODGliwice/zdjecia
				                    //}
									//alert("aaa");
    ...
    var gridDzialkWolnePanel = new Ext.grid.GridPanel({
		...
		var dsAnalizyDN = new GeoExt.data.FeatureStore({
				...
				fields: [
						...
						protocol: new OpenLayers.Protocol.HTTP({
								url: "RODGliwice/dajNiewymiarowe.php",
								format: new OpenLayers.Format.GeoJSON(),
								headers: {
		        ...
		        userArrows: true,
		      	...
		      	rootVisible: false,
		    		loader: new Ext.tree.TreeLoader({
		         		url  : 'RODGliwice/jsonTreeDok.php?dir='+activeGarden
		  	    }),
	...
	function dajDokumentyPozostalePanel(id){
		        ...
		        userArrows: true,
		      	...
		      	rootVisible: false,
		    		loader: new Ext.tree.TreeLoader({
		         		url  : 'RODGliwice/jsonTreeOtherDok.php'
		  	    }),
		    		root: new Ext.tree.AsyncTreeNode({
		    				id: 'isroot',
		    				expanded: true,
	...
	function dajWlascicielePanel(activeGarden){
				...
				autoScroll: true,
		    		loader: new Ext.tree.TreeLoader({
		         		url  : 'RODGliwice/jsonTreeOwner.php?gid='+activeGarden
		  	    }),
		    		root: new Ext.tree.AsyncTreeNode({
		    				id: 'isroot',
		    				expanded: true,
            ...
            handler:function(){
					...
					abc = wgrajPlikPanel.getForm().findField('um_komentarz').getValue();
							...
							//abc = "aaa",//wgrajPlikPanel.getForm().findField('um_komentarz').getValue(),
				            		url:'RODGliwice/wgrajPlik.php?nazwa='+wgrajPlikPanel.getForm().findField('um_komentarz').getValue()+'&id='+getCookie('usr'),
				                //method:'POST',
       ...
       buttons:[{
            ...
            handler:function(){
            		...
            		if(wgrajWMSPanel.getForm().isValid()){
						...
						Ext.Ajax.request({
			url: 'RODGliwice/dajWMSLayers.php?msg='+adr_wms,
			success: function(objServerResponse, opts) {
				var odp = Ext.decode(objServerResponse.responseText);
			...
			failure: function(){ alert('B£¥D');}
						...
						wgrajWMSPanel.getForm().submit({
							//abc = "aaa",//wgrajPlikPanel.getForm().findField('um_komentarz').getValue(),
				            		//url:'RODGliwice/wgrajPlik.php?nazwa='+wgrajWMSPanel.getForm().findField('um_komentarz').getValue()+'&id='+getCookie('usr'),
				                //method:'POST',
				               // waitMsg: 'Wgrywanie twojego pliku...',
		...
		//	success: function(form, opts) {
			...
			//}
						...
						/*                wgrajWMSPanel.getForm().reset();
							...
							warDodatkowe(),
								addLay();
							document.location="http://unimap.dyndns.biz/gliwice.html"
							//Ext.Msg.alert('Status', warstwyDodatkowe, function(btn, text){			 				                					                    
						    //            });	
				...
				if (feature.layer){
						switch (feature.layer.name)
						{
								case 'Zdjêcia Dzia³kowców':
										var adres = 'RODGliwice/wywalZdjecie.php?sciezka='+feature.attributes['sciezka'];
							      Ext.Ajax.request({
							      		url: adres,
							          success: function(objServerResponse, opts) {
							          		var odp = Ext.decode(objServerResponse.responseText);
		...
		function properAttrWin(feature){					
				...
				if((readCookie('upr')) && (readCookie('usr'))){
					...
					_upr = new uprawnienia(upr);
						...
						Ext.Ajax.request({
								url: "RODGliwice/infoDodatkowe.php?lname="+_lname+"&gid="+_gid,
								success: function(objServerResponse, opts) {									
										var odp = Ext.decode(objServerResponse.responseText);
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  	
		...
		function createPopupOgrodyFormB(feature){
				...
				Ext.Ajax.request({
						url: "RODGliwice/dajPoIndexie.php?co=miastodeleg&iddeleg="+iddeleg+"&idmiasta="+idmiasta,
						success: function(objServerResponse, opts) {									
								var odp = Ext.decode(objServerResponse.responseText);
				...
				*/
				...
				var miastaStore = new Ext.data.Store({
						...
						proxy: new Ext.data.HttpProxy({
								url: 'RODGliwice/dajSlownik.php?rodzaj=miasto'
						}),
						autoLoad: true,
						listeners: {
				...
				var delegStore = new Ext.data.Store({
						...
						proxy: new Ext.data.HttpProxy({
								url: 'RODGliwice/dajSlownik.php?rodzaj=deleg'
						}),
						autoLoad: true,
						listeners: {
								"load": function(store, rekordy, opcje) {
...
*/    
    ...
    miastaOgrody = new Ext.tree.TreePanel({
    		...
    		rootVisible: false,
    		loader: new Ext.tree.TreeLoader({
         		url  : 'RODGliwice/jsonTreeOgr.php'	
  	    }),
    		root: new Ext.tree.AsyncTreeNode({
    				id: 'isroot',
    				expanded: true,
...
/*    
    ...
    delegOgrody = new Ext.tree.TreePanel({
    		...
    		loader: new Ext.tree.TreeLoader({
         		url  : 'RODGliwice/jsonTreeDel.php'
  	    }),
    		root: new Ext.tree.AsyncTreeNode({
    				id: 'isroot',
    				expanded: true,
...
*/    
		...
		var analizy = new Ext.tree.TreePanel({
    		...
    		rootVisible: false,
    		loader: new Ext.tree.TreeLoader({
         		dataUrl  : 'RODGliwice/analizy.json',
    		...
    		if (_exe == true) {
					...
					setCookie('upr',readCookie('upr'),(15*60*1000));
						...
						_upr = new uprawnienia(upr);
				    				...
				    				Ext.Ajax.request({
												url: "RODGliwice/infoDodatkowe.php?lname=us&user="+_usr,
												success: function(objServerResponse, opts) {
														var odp = Ext.decode(objServerResponse.responseText);
    		...
    		if (_exe == true) {
					...
					setCookie('upr',readCookie('upr'),(15*60*1000));
						...
						_upr = new uprawnienia(upr);
		    				if (aktywnyOgrod){
				    				Ext.Ajax.request({
												url: "RODGliwice/infoDodatkowe.php?lname=o&gid="+aktywnyOgrod,
												success: function(objServerResponse, opts) {
														var odp = Ext.decode(objServerResponse.responseText);
														var deleg = odp.iddeleg;
														var uprD = _upr.getUprEl(deleg, 'd', 5);
	...
	var dsWlascicieleWD = new GeoExt.data.FeatureStore({
				...
				fields: [
						...
						protocol: new OpenLayers.Protocol.HTTP({
								refresh:true,
								url: 'RODGliwice/dajWlascicieli.php',
								format: new OpenLayers.Format.GeoJSON()
						})
				}), autoLoad: false
		}); 
	...
	var gridWlascicielePanel = new Ext.grid.GridPanel({
		...
		function uploadFile(){
    				...
    				if((getCookie('upr')) && (getCookie('usr'))){
	    				...
	    				Ext.Ajax.request({
							url: "RODGliwice/fileupload/file-upload.html",
							success: function(objServerResponse, opts) {
								var odp = Ext.decode(objServerResponse.responseText);
		...
		function analizaWlascicieleDzialek(_exe){
    		if (_exe == true) {
					...
					//if (_upr){
		    				if (aktywnyOgrod){
				    				Ext.Ajax.request({
												url: "RODGliwice/listaWlascicieli.php?lname=o&gid="+aktywnyOgrod,
												success: function(objServerResponse, opts) {
														var odp = Ext.decode(objServerResponse.responseText);
		...
		function analizaWlascicieleDzialekPDF(_exe){
    		...
    		if (_exe == true) {
    				...
    				//if (_upr){
		    				if (aktywnyOgrod){
				    			window.open('RODGliwice/listaWlascicieliPDF.php?&gid='+aktywnyOgrod,'','scrollbars=no,menubar=no,height=600,width=800,resizable=yes,toolbar=no,location=no,status=no');
		    				} else {
		    						Ext.Msg.alert('Niepowodzenie!', 'Brak aktywnego ogrodu, wybierz ogród i kliknij dwukrotnie w jego obrêbie');	
		    				}
		    		} else {
		...
		function analizaWlascicieleDzialekExcel(_exe){
    		...
    		if (_exe == true) {
    				...
    				//if (_upr){
		    				if (aktywnyOgrod){
				    			window.open('RODGliwice/listaWlascicieliExcel2.php?&gid='+aktywnyOgrod,'','scrollbars=no,menubar=no,height=600,width=800,resizable=yes,toolbar=no,location=no,status=no');
		    				} else {
		    						Ext.Msg.alert('Niepowodzenie!', 'Brak aktywnego ogrodu, wybierz ogród i kliknij dwukrotnie w jego obrêbie');	
		    				}
		    		} else {
